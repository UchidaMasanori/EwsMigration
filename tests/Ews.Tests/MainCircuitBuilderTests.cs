...
public sealed class MainCircuitBuilderTests
{
    ...
    [Fact]
    public void EleEqual_MCDT’P“Æ‚ÍFY685E()
    {
        // yCŒ´“TzMCDT ’P“Æ(“¯ˆê”Ô†‚ª2Œ‚Å‚È‚¢)‚Í step4 Yoyakugo_Check_MCDT ‚ªæ‚É FY-685E ‚ğo‚·
        //   ‚½‚ßAstep16 Ele_Equal_Check ‚Ì FY-630E(n != 1)‚É‚Í“’B‚µ‚È‚¢B
        var result = RunEleEqual(
            Kiki("MCDT", "1", 5, 0, ("p", "3")));
    ...
    [Fact]
    public void EleEqual_TSWd•¡‚ÍFY682E()
    {
        // yCŒ´“Tz“¯ˆê ysno ‚Ì TSW d•¡‚Í step4 Yoyakugo_Check_OTHER ‚ªæ‚É FY-682E(Œã‘±‹@ŠíˆÊ’u)‚ğo‚·
        //   ‚½‚ßAstep16 Ele_Equal_Check ‚Ì FY-630E ‚É‚Í“’B‚µ‚È‚¢B
        var result = RunEleEqual(
            Kiki("TSW", "1", 8, 0),
            Kiki("TSW", "1", 9, 0));
    ...
    {
        // yCŒ´“Tzysno==0(atoi)‚Í“¯ˆêƒ`ƒFƒbƒN‘ÎÛŠOBMC ’P“Æ(ysno=0)‚Í step4/step16 ‚Æ‚à‘f’Ê‚èB
        //   (MCDT ‚Ì ysno=0 ‚Í step4 Yoyakugo_Check_MCDT ‚ª FY-684E ‚ğo‚·‚½‚ß•Ê“r E.4 ‚ÅŒŸØB)
        var result = RunEleEqual(
            Kiki("MC", "0", 1, 0, ("p", "3")));
    ...
    // ==== E.4: Yoyakugo_Check_Double(‹@Šíî•ñŠÖ˜Aƒ`ƒFƒbƒN) ====

    /// <summary>‹@Šíƒe[ƒuƒ‹+síƒe[ƒuƒ‹‚Å MakeMain ‚ğÀs‚·‚éB</summary>
    ...
    /// <summary>síƒOƒ‹[ƒvNo•t‚«‚Å‹@Šíƒe[ƒuƒ‹1Œ‚ğ¶¬‚·‚éByCŒ´“TzKIKITABLE(G_No •t)B</summary>
    ...
    [Fact]
    public void Yoyakugo_RMCB“¯”Ô†d•¡‚ÍFY682E()
    {
        // yCŒ´“TzYoyakugo_Check_RM: RMCB ”Ô†•t‚«‚ÌŒã‘±“¯”Ô†d•¡ ¨ FY-682E(Œã‘±‹@ŠíˆÊ’u)B
        var result = RunEleEqual(
            Kiki("RMCB", "1", 3, 0),
            Kiki("RMCB", "1", 4, 0));
    ...
    [Fact]
    public void Yoyakugo_RRY‚Í“¯”Ô†d•¡‚ğ‹–—e‚·‚é()
    {
        // yCŒ´“TzYoyakugo_Check_RM: RRY ‚ÍœŠO(”Ô†d•¡‚ğ‹–—e)B
        var result = RunEleEqual(
            Kiki("RRY", "1", 3, 0),
            Kiki("RRY", "1", 4, 0));
    ...
    [Fact]
    public void Yoyakugo_MCDT”Ô†‚È‚µ‚ÍFY684E()
    {
        // yCŒ´“TzYoyakugo_Check_MCDT: —\–ñŒê‚¾‚¯(ysno==0)‚Í•s‰Â ¨ FY-684EB
        var result = RunEleEqual(
            Kiki("MCDT", "0", 6, 0));
    ...
    [Fact]
    public void Yoyakugo_MCDT“¯”Ô†‚ª3Œ‚ÍFY685E()
    {
        // yCŒ´“TzYoyakugo_Check_MCDT: “¯ˆê”Ô†‚Í’š“x2Œ‚Ì‚İ‹–‰Â(2Œ‚Å‚È‚¢¨ FY-685E)B
        var result = RunEleEqual(
            Kiki("MCDT", "1", 1, 0),
            Kiki("MCDT", "1", 2, 0),
            Kiki("MCDT", "1", 3, 0));
    ...
    [Fact]
    public void Yoyakugo_TR”Ô†•t‚«‚ÍFY683E()
    {
        // yCŒ´“TzYoyakugo_Check_TR: TR ‚É”Ô†•t‚«‚Í•s‰Â ¨ FY-683E(sí!=PS)B
        var result = RunEleEqual(
            Kiki("TR", "1", 7, 0));
    ...
    [Fact]
    public void Yoyakugo_TR”Ô†‚È‚µ‚Í³í()
    {
        // yCŒ´“TzYoyakugo_Check_TR: ”Ô†‚È‚µ TR ‚ÍƒGƒ‰[‚È‚µ(2“dŒ¹TRŒŸØ‚Í E.4b •Û—¯)B
        var result = RunEleEqual(
            Kiki("TR", "0", 7, 0));
    ...
    [Fact]
    public void Yoyakugo_SCˆÊ’u•s³‚ÍFY649E()
    {
        // yCŒ´“TzYoyakugo_Check_SC: sí‚ª PM/O/B/S/BO ˆÈŠO(‚±‚±‚Å‚Ísí–¢’è‹`=‹ó)‚Í FY-649EB
        var result = RunEleEqual(
            Kiki("SC", "0", 5, 0));
    ...
    [Fact]
    public void Yoyakugo_MC‚Æ“¯”Ô†MG‚ÍFY682E()
    {
        // yCŒ´“TzYoyakugo_Check_OTHER: MC ”Ô†•t‚«‚Æ“¯ˆê”Ô†‚Ì MG ‚Í•s‰Â ¨ FY-682E(MGˆÊ’u)B
        var result = RunEleEqual(
            Kiki("MC", "1", 2, 0),
            Kiki("MG", "1", 8, 0));
    ...
    [Fact]
    public void Yoyakugo_ˆê”Ê‹@Ší‚Ì“¯”Ô†d•¡‚ÍFY682E()
    {
        // yCŒ´“TzYoyakugo_Check_OTHER: MC/LGR/ELR/MCDT/CSDT/TR ˆÈŠO‚Ì”Ô†•t‚«Œã‘±d•¡ ¨ FY-682EB
        var result = RunEleEqual(
            Kiki("MCB", "1", 2, 0),
            Kiki("MCB", "1", 9, 0));
    ...
    [Fact]
    public void Yoyakugo_SCsíS‚Í³í()
    {
        // yCŒ´“TzYoyakugo_Check_SC: sí O/B/S/BO ‚Í‹–‰ÂB
        //   —LŒø‚Èsí\¬(Œn“1: P¨S)‚ğ—pˆÓ‚µASC ‚ğsíƒOƒ‹[ƒv(S)‚É‘®‚³‚¹‚éB
        var lineTypes = new[]
        {
    ...
    [Fact]
    public void Yoyakugo_SCsíPM‚Å’¼Œã“¯ˆêƒOƒ‹[ƒv‚ÍFY656E()
    {
        // yCŒ´“TzYoyakugo_Check_SC: sí PM ‚Å’¼Œã‹@Ší‚ª“¯ˆêƒOƒ‹[ƒvNo ¨ FY-656EB
        //   —LŒø‚Èsí\¬(Œn“1: P¨PM¨M)‚ğ—pˆÓB
        var lineTypes = new[]
        {
            Gyo(1, "P",  '1', 1, groupNumber: 1),
    ...
    [Fact]
    public void Yoyakugo_SCsíPM‚Å’¼Œã•ÊƒOƒ‹[ƒv‚Í³í()
    {
        // yCŒ´“TzYoyakugo_Check_SC: sí PM ‚Å’¼Œã‹@Ší‚ª•ÊƒOƒ‹[ƒvNo ¨ ‰Â(return 0)B
        var lineTypes = new[]
        {
            Gyo(1, "P",  '1', 1, groupNumber: 1),
            Gyo(1, "PM", '1', 2, groupNumber: 2),
