...
public class BranchArraySorterTests
{
    ...
    [InlineData(' ', '2')]
    [InlineData('2', '2')]
    public void ResolveEquipmentKindはkikiruiが1のときのみ1を返す(char kikirui, char expected)
    {
        Assert.Equal(expected, BranchArraySorter.ResolveEquipmentKind(Word(kikirui)));
    }

    [Fact]
    public void ResolveAttachedFunctionはALX単独登録で1を返す()
    {
        Assert.Equal('1', BranchArraySorter.ResolveAttachedFunction(Word(' ', Slot(1, "ALX"))));
    }

    [Fact]
    public void ResolveAttachedFunctionはAL単独登録で2を返す()
    {
        Assert.Equal('2', BranchArraySorter.ResolveAttachedFunction(Word(' ', Slot(1, "AL"))));
    }

    [Fact]
    public void ResolveAttachedFunctionは該当なしで3を返す()
    {
        Assert.Equal('3', BranchArraySorter.ResolveAttachedFunction(Word(' ', Slot(1, "MCB"))));
    }

    [Fact]
    public void ResolveAttachedFunctionはyukoidx0の枠で走査を打ち切る()
    {
        // 先頭枠 yukoidx=0 なので ALX があっても参照されず '3'。
        Assert.Equal('3', BranchArraySorter.ResolveAttachedFunction(Word(' ', Slot(0), Slot(1, "ALX"))));
    }

    [Fact]
    public void ResolveAttachedFunctionはALXの直後に登録があると一致しない()
    {
        // 【C原典】memcmp 12 は隔接要素まで読むため、ALX の後に非空白が続くと不一致。
    ...
    [InlineData("KY", '2')]
    [InlineData("CT", '3')]
    public void GetTypeKindFromComponentはptypeで種別を返す(string ptype, char expected)
    {
    ...
    [Fact]
    public void GetTypeKindFromComponentはSパワー系datatypeで4を返す()
    {
    ...
    [Fact]
    public void GetTypeKindFromComponentは該当なしで1を返す()
    {
    ...
    [Fact]
    public void SortIndexは予約語がテーブルにないとマイナス1を返す()
    {
    ...
    [Fact]
    public void SortIndexはKEY1をkikiruiKEY5を予約語種別から設定する()
    {
        ...
        Assert.Equal('3', klist[0].Key9); // タイプ枠未登録→'3'
    }

    [Fact]
    public void SortIndexはCompareSortIndex順に並べ替える()
    {
