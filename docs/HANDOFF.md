...
- **移植境界（重要）**: `Fysk00_Kikisearch_SY`（Fysk00.c, 大規模 ISAM 依存の機器選定本体・品番決定）は未移植のため、`Func<IReadOnlyList<MainCircuitResult>, int>` デリゲートで注入する境界とした。**移植アルゴリズム層（`Ews.Analysis` の Fyss* 群）では未移植の外部依存を interface でなく引数注入（デリゲート・CNS テーブル・ゾーンコード等）で表現する慣習**のため、それに合わせた。※これは正式なプロジェクト規約ではなく Ews.Analysis 層に限った慣習。インフラ層には interface が存在する（`IBatchJob`/`IIsamTable`/`IIsamRecord`/`IFacilityAreaResolver`/`IRuntimeParameterProvider`/`IPartNumberInfoRepository` の 6 件。「コードベース全体に interface は 0 件」は誤り）。
