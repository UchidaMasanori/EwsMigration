...
- **データ層**: `.cns` マスタ取込・ISAM 固定長エクスポート取込・SQL Server スキーマの本格整備。(1) 部署別仕様書一覧(siyosyo.cns)の階層パーサ＋`SpecificationKind`/`SpecificationFile` テーブル＋`SeedSpecificationMaster`。(2) **機器マスタ FYDM805**: `hostdt/FYDM805.data`(600byte/rec・LF区切り・Shift-JIS固定長)を `EquipmentMasterLoader.ParseEquipmentMaster`＋`SeedEquipmentMaster`(SqlBulkCopy)で取込。(3) **品番索引 FYDF816**: `hostdt/FYDF816.data`(184byte/rec・LF区切り、実データ11,424件・キー(品番+追番)一意)を `EquipmentPartNumberIndex.FromFixedRecord`＋`EquipmentMasterLoader.ParsePartNumberIndex`/`SeedPartNumberIndex`で取込。★ISAMは参照整合性を強制しないため `EquipmentPartNumberIndex`→`EquipmentMaster` の FOREIGN KEY は張らず結合用の非クラスタ索引のみ(`sql/001_schema.sql`)。★重要: 現状の `FYDM805.data` は予約語 "MCB" 付近で切れた**不完全エクスポート**(索引の予約語 MCDT/MG/MMCB/NT/OL/PBS/PT/RL/TB/WH/ZCT 等=全体の約48%が master に未存在)。→ **FYDM805.data の完全再エクスポートが必要**。未了: FYDM805完全再取得、siyosyo図面サイズ(`SiyosyoSizeCheck`)、printer.cns取込、機器マスタの補助情報/外形寸法など未展開フィールド。
