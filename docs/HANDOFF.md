...
### 2026-08-06 セッション 追加分（Fyss3D 使用相決定 段階移植(6) 統括本体末尾2ループ）

`Fyss3D_PH_Kettei` のメインP別相割付ループ直後の末尾2ループを `PhaseAssigner` へ移植。
...
### 2026-08-06 セッション 追加分（Fyss3D 使用相決定 段階移植(8) 1P3W-親1P3W3）

`Fyss3D_PH_Kettei` の 1P3W 電源ケース(~320行)を親機器種別に分割し、親 1P3W 極3 サブケースを移植。

- **`AssignParent1P3WPole3`**: 1P3W電源で親が1P3W極3の子機器(childIndex)の使用相をセット。子が1P3W3→親の使用相をコピー(MC/TBは除外)+SetPhaseMc2P/3P、子が1P2W2→XY、子が1P2W1→CollectF800Indexで同一親機器を収集しcount≤ならXN/YN交互(F直後WLはXN固定・SetPhaseMc)・count>ならA/BグループをSetPhase100VDevices、ヒューズは常にSetPhase100VDevices。想定外はreportDesignError(1)。★重複処理防止は HashSet<string>(親追番) を引数注入(原典sumi[0]/sumino[0])。mc_cntはref、hycpskbnはchar引数。原典 1+t[m]/j+1 参照は範囲ガード付(UB回避)。
- **テスト**: `PhaseAssignerTests.cs` +5件(SetKpaヘルパ追加)。テストは 1471 → **1476**（+5）。★★Fyss3D残り=1P3W-親1P2W2/親1P2W1サブケース・3P3W/3P4Wケース・初期化・統括ループ組立。

### 2026-08-06 セッション 追加分（Fyss3D 使用相決定 段階移植(9) 1P3W-親1P2W2）

`Fyss3D_PH_Kettei` の 1P3W 電源ケースの親 1P2W 極2 サブケースを移植。
...
### 2026-08-06 セッション 追加分（Fyss3D 使用相決定 段階移植(11) 3P3W電源）

`Fyss3D_PH_Kettei` の 3P3W 電源ケースを移植。

- **`AssignPhase3P3W`**→CircuitParseError?: Pの使用相=RST、j-loopで同系統子機器を親種別にディスパッチ。親3P3W3: 子3P3W3は親コピー・子1P2W2はF800Index33収集しcount<=2で先頭部SetPhase3P3WDevices+改訁29/30(3P混在&counta>=1&countf>=1で先頭2件RS固定)、count>2でta/tbをRS/ST/TR・末尾tf・MC子3Pは親極数/電圧/使用相コピー(改訁5)/else想定外reportDesignError(4)。親1P2W2(別if): MC子3Pは3P3W化RST(改訁5)/子1P2W2は親コピー/else reportDesignError(5)。プラグイン特注(hycpskbn 3/7 & datatype[0] CH/KP)はスキップ。dedupはHashSet。
- **テスト**: `PhaseAssignerTests.cs` +5件。テストは 1488 → **1493**（+5）。★★Fyss3D残り=3P4Wケース(v2なし/v2あり)・初期化・統括ループ組立。

### 2026-08-06 セッション 追加分（Fyss3D 使用相決定 段階移植(12) 3P4Wv0電源）

`Fyss3D_PH_Kettei` の 3P4W(回路電圧v2なし)電源ケースを移植。

- **`AssignPhase3P4WNoV2`**→CircuitParseError?: Pの使用相=RSTN、j-loopで同系統子機器を親種別の3つの別ifでディスパッチ。親3P4W4v0: 子3P4W4コピー/子3P3W3はRST/子1P2W2はF800Index34('1','2','2')収集後SetPhase3P3WDevices/子1P2W1はF800Index34('1','2','1')収集後SetPhase3P4WDevices/else reportDesignError(6)。親3P3W3(別if): 子3P3W3コピー/子1P2W2はF800Index34収集後SetPhase3P3WDevices/else reportDesignError(7)。親1P2W2(別if): 子1P2W2コピー/else reportDesignError(8)。dedupはsumino[2/3/4]に対応しHashSet×3。count<=2はt、>2はta/tb別にソート+相セット。
- **テスト**: `PhaseAssignerTests.cs` +7件。テストは 1493 → **1500**（+7）。★★Fyss3D残り=3P4Wケース(v2あり)・初期化・統括ループ組立。

### 2026-08-06 セッション 追加分（Fyss3D 使用相決定 段階移植(13) 3P4Wvあり電源）
...
### 段階17: Fyss3I_21_MCDT_CSDT 移植(2-1型MCDT/CSDT処理)

`Fyss15_Make_LowerParm` が呼ぶ 2-1 型 MCDT/CSDT 処理を `LowerParameterGenerator` に追加。
...
### 段階23: Fyss3C 残りの純粋収集関数(構成機器非依存)

構成機器(FYDF811 ― **未移植**)に依存しない残りの収集関数 2 つを移植。

- **`GetFloorElementsForSort`**(=GetFloorElementsForSortX): ソート対象の末端回路ブレーカを集める。doing・行種B/BO/O・盤種BN1/4・回路要素=1・直列追番=1・グループ親追番一致。
...
- **移植境界（重要）**: `Fysk00_Kikisearch_SY`（Fysk00.c, 大規模 ISAM 依存の機器選定本体・品番決定）は未移植のため、`Func<IReadOnlyList<MainCircuitResult>, int>` デリゲートで注入する境界とした。※未移植なのは Fysk00.c の**統括本体 `Fysk00_Kikisearch_SY` のみ**。同ファイルのサブ関数・改訂(Prop*)群（`Fysk00_ph`/`Fysk00_Make_Keiki`/`Fysk00_Set_VA_W`/`Fysk00_Get_Datachi`/`Fysk00_Area_Rewrite`/`Keiki_Check`/`LGR_Check`/`ZCT_Check`/`PropChgFuseType_SY`/`PropChgWlLampType`/`PropChgCTALMaker`/`PropChkHibknNum` 等 多数）は移植済み。「Fysk00 は全くの未移植」は誤り。**移植アルゴリズム層（`Ews.Analysis` の Fyss* 群）では未移植の外部依存を interface でなく引数注入（デリゲート・CNS テーブル・ゾーンコード等）で表現する慣習**のため、それに合わせた。※これは正式なプロジェクト規約ではなく Ews.Analysis 層に限った慣習。インフラ層には interface が存在する（`IBatchJob`/`IIsamTable`/`IIsamRecord`/`IFacilityAreaResolver`/`IRuntimeParameterProvider`/`IPartNumberInfoRepository` の 6 件。「コードベース全体に interface は 0 件」は誤り）。
