...
- **データ層**: `.cns` マスタ取込・ISAM 固定長エクスポート取込・SQL Server スキーマの本格整備。(1) 部署別仕様書一覧(siyosyo.cns)の階層パーサ `CnsMasterLoader.ParseSpecificationMaster`＋`SpecificationKind`/`SpecificationFile` テーブル＋`SeedSpecificationMaster`(全置換・KindSeq/FileSeq で配列順保持)。(2) **機器マスタ FYDM805** を移植: `EwsMigration/../hostdt/FYDM805.data`(旧 `FYDM805.af1` を 600byte/rec・LF 区切りの Shift-JIS 固定長へエクスポート、実データ 8,294 件・主キー重複0)を `EquipmentMasterLoader.ParseEquipmentMaster`(バイトオフセットで `EquipmentMaster.FromFixedRecord` へ委譲、244byte 未満の末尾断片は読み飛ばし)＋`SeedEquipmentMaster`(SqlBulkCopy で EquipmentMaster テーブルへ全置換)で取込。未了: 品番索引 **FYDF816** 取込 ※提供された `hostdt/FYDM816.data` は 4096byte で末尾が途中欠け(不完全)+レコード幅が 184 と不一致のため、完全再エクスポートが必要。siyosyo の図面サイズ(`SiyosyoSizeCheck`)、printer.cns 取込、機器マスタの補助情報/外形寸法など未展開フィールド。
