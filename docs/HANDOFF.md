...
- **データ層**: `.cns` マスタ取込・SQL Server スキーマの本格整備。部門(bumon)に加え **部署別仕様書一覧(siyosyo.cns)** の階層パーサ `CnsMasterLoader.ParseSpecificationMaster`（【C原典】`Zs20SiyoInfoRead`/`Fymzs40Cns.c`。部署→仕様書(名/説明/パス/ファイル[])。実マスタ10部署で動作確認済）を移植。未了: siyosyo の SQL スキーマ/Seed、図面サイズ(`SiyosyoSizeCheck`)、機器マスタ(FYDM805/ISAM)取込、`sql/001_schema.sql` の拡張。
