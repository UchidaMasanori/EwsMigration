...
回路解析（`toku/sekkei` 系）を先行移植中。**全 1038 テスト成功 / 0 スキップ / 0 失敗**。
...
**先頭ステップ群**を移植。オーケストレータ本体（約20サブ関数）は各サブを順次移植してから組み立てる方針。テストは 1018 → **1038**。
...
- **保留（Fyss15 残ステップ）**: `Fyss34_MattanGyouSento_Set`/`Fyss31_FukaHassei_Set`/`Fyss33_KikiSentei_Set`/`Fyss36`/`Fyss3G_Denryuu_Parm_Set`/`Fyss3B_Breaker_Sentei`(機器選定本体)/`Fyss3C`〜`Fyss3I`/`Fyss37`/`Fyss38`/`Fyss3A`/`Fyss3H`/`Fyss15_MCB1P_NT`/`Pre_CT_Make`。順次移植後にオーケストレータへ結線。
