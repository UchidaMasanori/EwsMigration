...
- **更新の粒度**:
  - **各増分（1 関数移植）ごとに** `docs/name-mapping.csv` とロードマップメモリ（`/memories/repo/ews-migration-roadmap.md`）を更新してコミットする。
  - **セッションの節目ごとに** 本ファイルの「5. 進捗」へまとめて反映し、コミット・`git push origin main` する。
