...
回路解析（`toku/sekkei` 系）を先行移植中。**全 276 テスト成功 / 1 スキップ / 0 失敗**。
...
- **電気パラメータ検証** `ElectricalParameterChecker`（`Fyss1d.c` の表駆動パーサ）
  ...
  - CT/VT付き('/')定格キー表 AM/VT/CT/RTR/BLTR/PLTR/THSW/WH を追加。next_1_get(NextOneGet)を移植し副記号 n_kigo を key_check へ伝搬(消費先 key_check_WH を移植済)。残る特殊展開 PT/BP は後続(TR は移植済み)
