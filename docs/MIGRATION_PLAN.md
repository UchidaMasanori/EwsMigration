...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **2353** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **788** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | （git log 参照） | アーク溶接機(YA)の基準電流算出 Get_Ibs_YA を LoadCurrentCalculator.CalculateArcWelder として移植。Get_Group でグループを解決し L=2.00/M=1.33/H=1.19 の係数で pow 計算。 |
| 最終更新 | 2026-08-13 | — |
