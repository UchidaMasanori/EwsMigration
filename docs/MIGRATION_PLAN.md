...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **2361** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **789** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | （git log 参照） | 電動機(M)の基準電流算出 Get_Ibs_M を LoadCurrentCalculator.CalculateMotor として移植。相数/電圧/始動区分で yno、容量とグループで tno を決めテーブル grp_l/m/h の指数・係数で pow 計算。 |
| 最終更新 | 2026-08-13 | — |
