...
| 直近コミット | `TBHASH` | Fyss14 SetParam_ep2(list版) に TB(端子台) case を追加。記述行/系統を走査して端子台極数 epap を優先順に決定(直列トリップMCSD等->6・シャッターMGSHは電源相線で6/5/4・27A27B27C->2/2/3且自身 tokkbn=6・MC回路N相共用TBKY->1/2・基本は相数線式で2/3/3/4)し、V2 に回路電圧最大値を格納。Parm_Set_TB_Chk1〜4 をヘルパー化 |
| 最終更新 | 2026-08-06 | — |
