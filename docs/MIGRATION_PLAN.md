...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1307** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **550** | — |
...
| ファイル(行) | 役割 | 状況 |
|---|---|---|
| **Fyss3G(4073)**, **Fyss3D(2901)**, **Fyss3C(2397)**, Fyss3R(586), Fyss3F(370), Fyss3A(363),3B(211),3E(213),3H(317),3I(168) | 回路設計処理/線番系統変更 | 🟡 **Fyss3G ✅完全移植完了**（CNS/Seek/Check/全セッタ/ディスパッチャ本体）/ **Fyss3B ✅完全移植完了**（フラグ設定部＋本体オーケストレータ。末尾 Fysk00_Kikisearch_SY は未移植のためデリゲート注入境界）/ 他は ❌ |
