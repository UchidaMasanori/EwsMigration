...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1651** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **661** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | `e547773` | 段階31 Fyss15_Make_LowerParm オーケストレータ本体組立(全calleeをC呼出順で配線・CT生成で戻り-1・外部依存は引数注入)→Fyss15下流パラメータMAIN完成 |
| 最終更新 | 2026-08-06 | — |
