...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **2198** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **761** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | （git log 参照） | 構成機器レコード位置取得 Fysk01_Copy_Rec_Get(Fysk01.c:3482) を ComponentRecordFinder.FindByKey として移植。構成機器エリア(FYRT804 配列)の先頭 n 件を走査し検索キー先頭 10 バイトが一致する最初の位置を返す(該当なしは -1)。FYRT804 完全モデル未整備のため ComponentBufferInserter と同じジェネリック+キー抽出関数で表現(固定長前方一致)。テスト+6(2192→ 2198)。 |
| 最終更新 | 2026-08-08 | — |
