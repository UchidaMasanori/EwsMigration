...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1834** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **702** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | `SWITCHTYPEHASH` | Fyss14 の切り換えタイプ設定 CS_MCDT_12_21_SET を SwitchTypeSetter として移植。予約語 CSDT/MCDT かつ予約語指定番号 ysno!="00" かつ切り換えタイプ未設定の要素について、同一予約語・同一 ysno・未設定の後続要素と対を作り、親データ追番 oyatno 一致で 1-2型('1')・不一致で 2-1型('2')を双方へ設定する。親が異なるのに系統番号 kno が同一なら FY-922E を返す(950426/960404)。Fyss14_Make_UpperParm のループ後処理群のうちの 1 つ。テスト+7 |
| 最終更新 | 2026-08-06 | — |
