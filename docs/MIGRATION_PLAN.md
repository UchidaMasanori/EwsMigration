...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1325** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **552** | — |
...
| ファイル(行) | 役割 | 状況 |
|---|---|---|
| **Fyss3G(4073)**, **Fyss3D(2901)**, **Fyss3C(2397)**, Fyss3R(586), Fyss3F(370), Fyss3A(363),3B(211),3E(213),3H(317),3I(168) | 回路設計処理/線番系統変更 | 🟡 **Fyss3G ✅完全移植完了**（CNS/Seek/Check/全セッタ/ディスパッチャ本体）/ **Fyss3B ✅完全移植完了** / **Fyss3R 🟡基盤部移植**（プラグインタイプ判定 FyHcPlugInJdgType＋電源分岐グルーピング PropGrouping。結線相セット/主幹チェックは未）/ 他は ❌ |
