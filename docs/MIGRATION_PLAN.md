...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1686** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **670** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | `HASH_PLACEHOLDER` | Fyss14 SetParam_ep2(list版) に MC 子機エレメント修正 PropMcChildElement(改訂<6>/<8>/<9>/<10>/<16>) を追加し、MC のテスト・name-mapping をバックフィル。子機(SB/MCB/ELB)のエレメント数 ep[2].E を負荷電圧 fpalv[0] で決定(200=2/100=1/000は親回路電圧・極数で判定)、子機3Pは線式・極数を 3 に |
| 最終更新 | 2026-08-06 | — |
