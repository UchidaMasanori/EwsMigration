...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1706** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **679** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | `RTRHASH` | Fyss14 SetParam_ep2(list版) に RTR(計器用変流器付トランス) case を追加。ep[0].epav2 の定格電圧を自身回路電圧に据え相数・線式・極数・区分・周波数を親から複写、V1 は RTR_V1(=SetPltr 再利用)・V2 は新規 SetTrV2(SetParam_ep2_TR_V2: 1/2 電源トランス判別と同一 TR 前後関係で電圧スロット振分)。引数 Helutzu/pprmp/newpprmp 未使用(物件非依存)。併せて SetPltr の epav1[0] 初期値バグ(零値 "000000.0" を Fyss17/Fyss40 の照合で確認)を修正し PLTR テスト期待値を訂正 |
| 最終更新 | 2026-08-06 | — |
