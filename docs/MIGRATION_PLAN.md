...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **2395** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **792** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | （git log 参照） | タイプ別 AT/AF/MA/AM 設定 Fysk01_Set_ATAFMA を AtAfMaAmSetter.Apply として移植。epno==2 は Get_Ibs 基準電流を設定、それ以外は入力優先・下位フォールバックで設定。 |
| 最終更新 | 2026-08-13 | — |
