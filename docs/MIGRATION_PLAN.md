...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **2279** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **774** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | （git log 参照） | 基準値の中点距離比較 Fysk01_Choki_Cmp2(Fysk01.c:2779) を RangeMidpointComparer.Compare として移植。基準値が今回/前回2つの幅データ範囲のどちらでより中点に近いか=|中点-基準|で比較し今回<前回で1(入れ替える)、下限>=上限または基準範囲外で-1(SYS_ERR)。Choki_Cmp1の兄弟(THSW/TM用・中点距離)で純DOUBLE演算・依存ゼロ。テスト+8(2271→ 2279)。 |
| 最終更新 | 2026-08-13 | — |
