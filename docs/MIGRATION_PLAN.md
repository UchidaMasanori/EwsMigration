...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **2178** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **757** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | （git log 参照） | INV 直近上位 kw 選定 Fysk01_ChkInvKw(Fysk01.c:5567 改訂<28>) を InverterKwSelector.SelectKw として移植。コンスタント inv_prm(新モデル InverterConstant=type[7][7]+kw) を上から走査しタイプ 2(dtype[1]) 一致行のうち入力 kw 以上となる最初の kw を返す。一致タイプ帯を通り過ぎたら打切り・該当なしは 0.0。ISAM 非依存の純走査リーフ。テスト+6(2172→ 2178)。 |
| 最終更新 | 2026-08-08 | — |
