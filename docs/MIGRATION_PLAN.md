...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1676** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **667** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | `HASH_PLACEHOLDER` | Fyss14 SetParam_ep2(list版) に PLTR を追加(SetParam_ep2_RTR_V1)。親(RTRなら更にその親)の回路電圧 kv0 で 1 次側電圧 V1[0] を決定(>105->200/以下->100/PLTRかつ>=380->400)し VC 区分に計器 1 次側電圧区分を設定 |
| 最終更新 | 2026-08-06 | — |
