...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1004** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **488** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~12〜15%** | 100% |
| 直近コミット | `2f7ea12` | — |
| 最終更新 | 2026-07-31 | — |
...
| Fyss30-39(≈5000): Fyss31(1076),33(572),36(832),37(939),39(757) ほか | 制御発生設定 等 | ❌ |
...
| ファイル(行) | 役割 | 状況 |
|---|---|---|
| **Fyss3G(4073)**, **Fyss3D(2901)**, **Fyss3C(2397)**, Fyss3R(586), Fyss3F(370), Fyss3A(363),3B(211),3E(213),3H(317),3I(168) | 回路設計処理/線番系統変更 | ❌ |
...
1. C 版 `libfysek.a` を `test_tool/FyskMain.c` でビルドし、代表物件の入力(FYDF805/801/802) →
   出力(FYRT800/801/802/805)を固定長ダンプ。
2. 同一入力を C# パイプラインに与え、出力をバイト比較（CP932）。
3. 差分を回路単位で自動レポート。段階別（Fyss11 後/Fyss12 後/…）の中間スナップショット比較で局所化。
4. 各フェーズ完了の受け入れ条件 = 該当段階までの中間出力一致。
