...
| 直近コミット | `2460417` | 制御仕様テーブル作成 MakeSgsTable(Fyss1k.c:254) を ControlSpecTableBuilder.MakeSgsTable として移植。系統(KEITOU)/行種(GYOSYU)/制御仕様文字列から制御仕様テーブル(FYRT820)の 1 レコードを構築。系統種別(1/3=P系統'1'/4=UP系統'2')、行種コード(左詰め空白埋め3桁)、行種番号(右詰め'0'埋め2桁)、記述行(atoi)、名称追番(追加後 1 始まり件数)を set。C の realloc は List 追加で表現。テスト+13(2092→ 2105)。 |
| 最終更新 | 2026-08-07 | — |
