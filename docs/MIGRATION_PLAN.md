...
| 直近コミット | （git log 参照） | 基準値の中央寄り比較 Fysk01_Choki_Cmp1(Fysk01.c:2734) を RangeCenteringComparer.Compare として移植。基準値が今回/前回2つの幅データ範囲のどちらでより中央寄りかを偏り=|((基準-下限)-(上限-基準))/幅|で比較し今回<前回で1(入れ替える)、下限>=上限または基準範囲外で-1(SYS_ERR)。THR/MG/XERY用の純DOUBLE演算・依存ゼロ。テスト+8(2263→ 2271)。 |
| 最終更新 | 2026-08-13 | — |
