...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **2183** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **758** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | （git log 参照） | INV 別パラメータ直近上位 kw 検索 Fysk01_ChkInvKwPara(Fysk01.c:5618 改訂<27>) を InverterKwSelector.SelectKwByParameter として移植。タイプ 1 番目からの一致桁数(i*7)を i=7から1 と緩めながら走査し各段で入力 kw 以上の最初の行の kw/タイプで上書き(外側は打切らず最も緩い一致段が優先)。選択タイプも返す。ISAM 非依存の純走査リーフ(ChkInvKw と対)。テスト+5(2178→ 2183)。 |
| 最終更新 | 2026-08-08 | — |
