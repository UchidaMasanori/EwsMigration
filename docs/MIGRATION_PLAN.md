...
| 指標 | 現在値 | 総量/目標 |
|---|---|---|
| テスト成功数 | **1810** | 0 失敗 / 0 スキップを維持 |
| 移植エントリ数（name-mapping.csv 行数） | **698** | — |
| 推定移植率（libfysek.a ~110k + libfysgy.a ~67k ≒ 177k 行） | **~14〜17%** | 100% |
| 直近コミット | `MAINRANKHASH` | Fyss14 の系統座標付与 Main_Rank_Set を SystemRankAssigner として移植。系統(ksyubetu=1)内の各要素へ入線番号・生成回路番号・直列追番・並列追番・上流並列追番・親データ追番・グループ親データ追番・グループ並列追番を後方走査で採番する。冒頭で CircuitNumberGenerator を生成し直し（Kbangoua/PM_flg の memset クリア相当）行種グループ番号 gyoglno と積算ワーク wk を初期化。指示フラグ narakbn=2/4 は rank[階層-1] 配列で並列追番を採番し、通常行は同一階層(941205)で前要素から連番、RRY 特例(改訂15)・96.03.07 の親データ追番特例を含む。テスト+10 |
| 最終更新 | 2026-08-06 | — |
